...
public class BeatsMover : MonoBehaviour
{
    ...
    private void Start()
    {
        GameManager.instance.audioSource.time = 0;
                ...
                switch (beat.beatType)
                {
                    case BeatType.SinglePress:                        
                        if (GetTiming(beat) == 0)
                        {
                            ...
                            mover.transform.position = Lerp(beatsOrigin.transform.position, notesParent.GetChild(((SinglePressBeat)beat).note - 1).transform.position, 1 - (beatsTime - currentTime) / (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)));
                        }
                    ...
                    case BeatType.Hold:
                        ...
                        if (GetTiming(beat) == 0)
                        {
                            ...
                            {
                                mover.transform.GetChild(0).position = Lerp(beatsOrigin.transform.position, notesParent.GetChild(((HoldBeat)beat).note - 1).transform.position, 1 - (beatsTime - currentTime) / (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)));
                            }
                            else
                            {
                            ...
                            if (beatsTime + (((HoldBeat)beat).duration * 60 / GameManager.instance.bpm) - currentTime <= (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)))
                            {                                
                                mover.transform.GetChild(1).position = Lerp(beatsOrigin.transform.position, notesParent.GetChild(((HoldBeat)beat).note - 1).transform.position, 1 - (beatsTime + (((HoldBeat)beat).duration * 60 / GameManager.instance.bpm) - currentTime) / (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)));
                            }
                            else
                            {
                    ...
                    case BeatType.Scratch:
                        ...
                        if (GetTiming(beat) == 0)
                        {
                            ...
                            for (int i = 0; i < 6; i++)
                            {
                                mover.transform.GetChild(i).position = Lerp(beatsOrigin.transform.position, notesParent.GetChild(i).transform.position, 1 - (beatsTime - currentTime) / (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)));
                            }
                        }
                        else if (beat.mover != null)
                        {
                    ...
                    case BeatType.DoublePress:
                        ...
                        if (GetTiming(beat) == 0)
                        {
                            ...
                            for (int i = 0; i < 2; i++)
                            {
                                mover.transform.GetChild(i).position = Lerp(beatsOrigin.transform.position, notesParent.GetChild(((DoublePressBeat)beat).notes[i] - 1).transform.position, 1 - (beatsTime - currentTime) / (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)));
                            }
                        }
                        else if (beat.mover != null)
                        {
    ...
    public int GetTiming(Beat beat) 
    {
        ...
        switch (beat.beatType)
        {
            ...
            case BeatType.SinglePress:
                ...
                // the time it requires for the beat to make it to the note is 1 / (beatSpeed * beat.speed * 60 / GameManager.instance.bpm), so if beatsTime - currentTime is greater than that, then the beat would reach the note too quickly
                if (beatsTime - currentTime > (1 / (beatSpeed * beat.speed * GameManager.instance.bpm / 60)))
                {
                    return -1;
                }
            ...
            case BeatType.Hold:
                ...
                else if (beatsTime + (((HoldBeat)beat).duration * 60 / GameManager.instance.bpm) + beatDelay - currentTime < 0)
                {
                    return 1;
                }
                else
    ...
    {
        return beat.beatAt / Beats.instance.quarterBeatsPerSecond;
    }
