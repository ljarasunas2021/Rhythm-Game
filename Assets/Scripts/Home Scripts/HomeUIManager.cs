...
public class HomeUIManager : MonoBehaviour
{
    ...
    {        
        ...
        foreach (AudioClipAndBPM clipAndBpm in GameManager.instance.audioClips)
        {
            songDropdown.options.Add(new Dropdown.OptionData(clipAndBpm.audioClip.name));
        }

        RefreshInputMenu();
        OnChangeDropdown();
        OnChangeSongDropdown();
    }
    ...
    public void CreateNew()
    {
        if (createNewIF.text != "")
        {
            GameManager.instance.SetSave(new Save(createNewIF.text, new List<Beat>(), selectedSong, GameManager.instance.bpm));
            SceneManager.LoadScene("Editor");
        }
    }
    ...
    public void OnChangeSongDropdown()
    {
        ...
        GameManager.instance.bpm = GameManager.instance.audioClips[selectedSong].bpm;
    }
